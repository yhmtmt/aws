...
* (at your option) any later vfersion.
*
...
Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight, const long long &timeStamp, ORBextractor* extractorLeft, ORBextractor* extractorRight, ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth)
...
Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const long long &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth)
...
Frame::Frame(const cv::Mat &imGray, const long long &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth)
    :mpORBvocabulary(voc),mpORBextractorLeft(extractor),mpORBextractorRight(static_cast<ORBextractor*>(NULL)),
     mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth)
{
	mGrid = new std::vector<std::size_t>*[mFrameGridCols];
...
bool Frame::isInFrustum(MapPoint *pMP, float viewingCosLimit)
{
   ...
   // Check viewing angle
    ...
    int nPredictedLevel = pMP->PredictScale(dist,mfLogScaleFactor);
	int nMaxLevels = (int)mvScaleFactors.size();

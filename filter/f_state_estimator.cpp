...
bool f_state_estimator::proc()
{
  ...
  double gps_xecef, gps_yecef, gps_zecef;
  Mat Renu;
  m_ch_state->get_position(tpos, gps_lat, gps_lon,
			   gps_xecef, gps_yecef, gps_zecef, Renu);
  ...
  if (tpos > m_tpos_prev){
    double dtx = ((tpos - m_tpos_prev) * (1.0 / (double)SEC));
    double dtv = ((tpos - m_tvel_prev) * (1.0 / (double)SEC));
    ...
    double xp = m_u_opt * dtx, yp = m_v_opt * dtx;
    Mat Kx = Pdtx * (m_Rx + Pdtx).inv();
    double x, y, z; // observed x, y, z
    eceftowrld(m_Renu_opt, m_xecef_opt, m_yecef_opt, m_zecef_opt, gps_xecef, gps_yecef, gps_zecef, x, y, z);
    
    double ex = (x - xp);
    double ey = (y - yp);
    double * pK = Kx.ptr<double>(0);
    double xe = (pK[0] * ex + pK[1] * ey + xp);
    double ye = (pK[2] * ex + pK[3] * ey + yp);

    double x_opt, y_opt, z_opt;
    ...
    if (m_blog){
      log_pos(t, tpos, gps_lat, gps_lon, 0, gps_xecef, gps_yecef, gps_zecef);
    }
  }
  

  if (tvel > m_tvel_prev){
    double dtx = ((tvel - m_tpos_prev) * (1.0 / (double)SEC));
    double dtv = ((tvel - m_tvel_prev) * (1.0 / (double)SEC));

    Mat Pdtv;
    Pdtv = dtv * m_Qv + m_Pv;

    Mat Kv = Pdtv * (m_Rv + Pdtv).inv();
    double cog_rad = (cog * (PI / 180.));
    double cog_cos = cos(cog_rad);
    double cog_sin = sin(cog_rad);
    double sog_mps = (sog * (1852. / 3600.));
    double u = (sog_mps * cog_sin), v = (sog_mps * cog_cos);
    m_cog_prev = cog;
    m_sog_prev = sog;
    m_u_prev = u;
    m_v_prev = v;
    m_Pv = (Mat::eye(2, 2, CV_64FC1) - Kv) * Pdtv;
    float eu = (float)(u - m_u_opt), ev = (float)(v - m_v_opt);
    double * pK = Kv.ptr<double>(0);
    ...
    if (m_bverb){
      cout << "tsys:" << m_cur_time << " t:"
	   << tvel << " (u,v)=(" << u << "," << v << ") (uopt,vopt)=(" 
...
void f_state_estimator::log_pos(const long long t, const long long tpos, 
	...
	const double gps_xecef, const double gps_yecef, const double gps_zecef)
{
  double * p = m_Px_ecef.ptr<double>();
  m_flog_x << t << "," << tpos << ","

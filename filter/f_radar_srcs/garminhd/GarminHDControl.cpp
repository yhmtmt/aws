...
bool GarminHDControl::Init(std::string & name, NetworkAddress &ifadr, NetworkAddress &radaradr) {
  int r;
  int one = 1;
...
void GarminHDControl::logBinaryData(std::string &what, const void *data, int size) {
  wxString explain;
...
bool GarminHDControl::TransmitCmd(const void *msg, int size) {
  ...
  //  IF_LOG_AT(LOGLEVEL_TRANSMIT, logBinaryData(wxString::Format(("%s transmit"), m_name), msg, size));
  return true;
}

void GarminHDControl::RadarTxOff() {
  //  IF_LOG_AT(LOGLEVEL_VERBOSE | LOGLEVEL_TRANSMIT, wxLogMessage(("radar_pi: %s transmit: turn off"), m_name));
...
void GarminHDControl::RadarTxOn() {
  //  IF_LOG_AT(LOGLEVEL_VERBOSE | LOGLEVEL_TRANSMIT, wxLogMessage(("radar_pi: %s transmit: turn on"), m_name));
...
bool GarminHDControl::SetControlValue(ControlType controlType, int value, RadarControlState state) {
  bool r = false;
  ...
  switch (controlType) {
    ...
    case CT_FTC: {
      printf(("radar_pi: %s FTC: value=%d"), m_name.c_str(), value);

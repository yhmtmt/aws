...
#include "RadarType.h"
  ...
  if (n == 0) {
    printf("Internal error: Programmer forgot to define ranges for radar type %d units %d", (int)radarType, (int)units);
    exit(1);
  }
  return n;
}

const char * RadarFactory::GetRadarTypes(const size_t radar_type) {
  const char * names[] = {
#define DEFINE_RADAR(t, x, s, l, a, b, c, d) x,
#include "RadarType.h"
  };

  return names[radar_type]
}

...
void NavicoReceive::ProcessFrame(const uint8_t *data, size_t len) {
  ...
  for (size_t scanline = 0; scanline < scanlines_in_packet; scanline++) {
    ...
    if (line->common.headerLen != 0x18) {
      printf(("radar_pi: strange header length %d"), line->common.headerLen);
    ...
    if (line->common.status != 0x02 && line->common.status != 0x12) {
      printf(("radar_pi: strange status %02x"), line->common.status);
...
SOCKET NavicoReceive::GetNewReportSocket() {
  ...
  SOCKET socket;
  std::string error = wxT("");
  std::string s = wxT("");

  if (m_interface_addr.IsNull() || m_report_addr.IsNull()) {
    printf(("radar_pi: %s no address to listen on"), m_ri->m_name);
  ...
  if (socket != INVALID_SOCKET) {
    ...
    printf(("radar_pi: %s scanning interface %s for data from %s"), m_ri->m_name, addr.c_str(), rep_addr.c_str());
    ...
    s << error;
    SetInfoStatus(s);
    printf(("radar_pi: %s Unable to listen to socket: %s"), m_ri->m_name, error.c_str());
  }
  return socket;
}
...
SOCKET NavicoReceive::GetNewDataSocket() {
  ...
  if (socket != INVALID_SOCKET) {
    ...
    printf(("radar_pi: %s listening for data on %s from %s"), m_ri->m_name.c_str(), addr.c_str(), rep_addr.c_str());
  } else {
    SetInfoStatus(error);
...
void *NavicoReceive::Entry(void) {
  ...
  printf(("radar_pi: NavicoReceive thread %s starting"), m_ri->m_name.c_str());
  ...
  while (m_receive_socket != INVALID_SOCKET) {
    ...
    printf(("radar_pi: select maxFd=%d r=%d elapsed=%lld"), maxFd, r, wxGetUTCTimeMillis() - start);
          ...
          if (ProcessReport(data, (size_t)r)) {
            if (!radar_addr) {
              ...
              if (m_ri->m_state.GetValue() == RADAR_OFF) {
                printf(("radar_pi: %s detected at %s"), m_ri->m_name.c_str(), radar_address.FormatNetworkAddress());
    ...
    if (!radar_addr && m_pi->m_locator) {
      ...
      if (me) {
        ...
        UpdateSendCommand();
        printf(("radar_pi: Locator found radar %u at IP %s [%s]"), m_ri->m_radar, ip.FormatNetworkAddressPort(),
                 me->to_string());
      }
...
static void AppendChar16String(std::string &dest, uint16_t *src) {
  for (; *src; src++) {
...
bool NavicoReceive::ProcessReport(const uint8_t *report, size_t len) {
  ...
  if (report[1] == 0xC4) {
        ...
        if (s->radar_status != m_radar_status) {
          ...
          switch (m_radar_status) {
            ...
            case 0x05:
              ...
              break;
            default:
              logBinaryData("received unknown radar status"), report, len);
              stat = _("Unknown status");
              break;
          }

	char buf[32];
	snprintf(buf, 32, ("IP %s %s"), m_pi->m_settings.radar_address[m_ri->m_radar].FormatNetworkAddress(), stat.c_str()); 
	std::string s = buf;
          if (RadarOrder[m_ri->m_radar_type] >= RO_PRIMARY) {
            NavicoRadarInfo info = m_pi->GetNavicoRadarInfo(m_ri->m_radar);
            s << ("\n") << ("Serial #") << info.serialNr;
          }
          SetInfoStatus(s);
        }
        break;
      ...
      case (99 << 8) + 0x02: {  // length 99, 02 C4
        ...
        printf(("radar_pi: %s state range=%u gain=%u sea=%u rain=%u if_rejection=%u tgt_boost=%u tgt_expansion=%u"),
      ...
      case (129 << 8) + 0x03: {  // 129 bytes starting with 03 C4
        ...
        printf(("radar_pi: %s RadarReport_03C4_129 radar_type=%u"), m_ri->m_name.c_str(), s->radar_type);

        switch (s->radar_type) {
          case REPORT_TYPE_BR24:
            if (m_ri->m_radar_type != RT_BR24) {
              printf(("radar_pi: Radar report tells us this a Navico BR24"));
              SetRadarType(RT_BR24);
            }
            break;
          case REPORT_TYPE_3G:
            if (m_ri->m_radar_type != RT_3G && m_ri->m_radar_type != RT_BR24) {
              printf(("radar_pi: Radar report tells us this an old Navico 3G, use BR24 instead"));
              SetRadarType(RT_BR24);
            }
            break;
          case REPORT_TYPE_4G:
            if (m_ri->m_radar_type != RT_4GA && m_ri->m_radar_type != RT_4GB && m_ri->m_radar_type != RT_3G) {
              printf(("radar_pi: Radar report tells us this a Navico 4G or a modern 3G"));
          ...
          case REPORT_TYPE_HALO:
            if (m_ri->m_radar_type != RT_HaloA && m_ri->m_radar_type != RT_HaloB) {
              printf(("radar_pi: Radar report tells us this a Navico HALO"));
            ...
            break;
          default:
            printf(("radar_pi: Unknown radar_type %u"), s->radar_type);
            return false;
        }
      ...
      case (66 << 8) + 0x04: {  // 66 bytes starting with 04 C4
        if (m_pi->m_settings.verbose >= 2) {
          logBinaryData(("received RadarReport_04C4_66"), report, len);
        }
      ...
      case (564 << 8) + 0x05: {  // length 564, 05 C4
        ...
        printf(("received familiar BR24 report"), report, len);
      ...
      case (21 << 8) + 0x08: {  // length 21, 08 C4
        ...
        printf(("%u 08C4: doppler=%d speed=%d"), m_ri->m_radar, s08->doppler_state, s08->doppler_speed);
        // TODO: Doppler speed
      ...
      case (18 << 8) + 0x08: {  // length 18, 08 C4
        ...
        printf(("%u 08C4: scanspeed=%d noise=%u target_sep=%u"), m_ri->m_radar, s08->scan_speed, s08->noise_rejection,
                    s08->target_sep);
        printf(("%u 08C4: f2=%u f6=%u f7=%u f8=%u f10=%u"), m_ri->m_radar, s08->field2, s08->field6, s08->field7,
                    s08->field8, s08->field10);
        printf(("%u 08C4: f11=%u f12=%u f13=%u f14=%u"), m_ri->m_radar, s08->field11, s08->field12, s08->field13,
                    s08->field14);
        printf(("%u 08C4: if=%u slsa=%u sls=%u"), m_ri->m_radar, s08->local_interference_rejection, s08->sls_auto,
        ...
        if (m_pi->m_settings.verbose >= 2) {
          logBinaryData(("received RadarReport_08C4_18"), report, len);
        }
        break;
      }
      ...
      case (66 << 8) + 0x12: {  // 66 bytes starting with 12 C4
        ...
        std::string sn = "#";
        sn << s->serialno;
        printf(("radar_pi: %s serial number is: %s"), m_ri->m_name.c_str(), sn);
        printf(("radar_pi: %s RadarReport_12C4_66 serialno=%s"), m_ri->m_name.c_str(), sn);
        break;
      }

      default: {
        if (m_pi->m_settings.verbose >= 2) {
          logBinaryData(("received unknown report"), report, len);
        }
        break;
      }
    }
    ...
    switch ((len << 8) + report[0]) {
      case (16 << 8) + 0x0f:
        if (m_pi->m_settings.verbose >= 2) {
          logBinaryData("received BR24 report"), report, len);
        }
        if (m_ri->m_radar_type == RT_UNKNOWN) {
          printf(("radar_pi: Radar report tells us this a Navico BR24"));
          m_ri->m_radar_type = RT_BR24;
      ...
      case (46 << 8) + 0x13:
        // Content unknown, but we know that BR24 radomes send this
        if (m_pi->m_settings.verbose >= 2) {
          logBinaryData(("received familiar report"), report, len);
        }
        break;
    ...
    if (m_pi->m_settings.verbose >= 2) {
      logBinaryData("received RadarReport_01B2"), report, len);
    }
    RadarReport_01B2 *data = (RadarReport_01B2 *)report;

#define LOG_ADDR_N(n) printf(("addr%d = %s"), n, FormatPackedAddress(data->addr##n));

...
bool NavicoControl::Init(NetworkAddress &ifadr, NetworkAddress &radaradr) {
  int r;
  int one = 1;
...
void NavicoControl::logBinaryData(std::string & what, const uint8_t *data, int size) {
  wxString explain;
  int i = 0;

  explain.Alloc(size * 3 + 50);
  explain += ("radar_pi: ") + m_name.c_str() + (" ");
  explain += what;
  char valstr[32];
...
bool NavicoControl::TransmitCmd(const uint8_t *msg, int size) {
  ...
  //IF_LOG_AT(LOGLEVEL_TRANSMIT, logBinaryData(("transmit"), msg, size));
  return true;
}

void NavicoControl::RadarTxOff() {
  //IF_LOG_AT(LOGLEVEL_VERBOSE | LOGLEVEL_TRANSMIT, wxLogMessage(("radar_pi: %s transmit: turn Off"), m_name.c_str()));
...
void NavicoControl::RadarTxOn() {
  //IF_LOG_AT(LOGLEVEL_VERBOSE | LOGLEVEL_TRANSMIT, wxLogMessage(("radar_pi: %s transmit: turn on"), m_name.c_str()));
...
bool NavicoControl::RadarStayAlive() {
  printf(("radar_pi: %s transmit: stay alive"), m_name.c_str());
...
bool NavicoControl::SetControlValue(ControlType controlType, int value, RadarControlState state) {
  ...
  switch (controlType) {
    ...
    case CT_BEARING_ALIGNMENT: {  // to be consistent with the local bearing alignment of the pi
      ...
      printf(("radar_pi: %s Bearing alignment: %d"), m_name.c_str(), v);
    ...
    case CT_GAIN: {
      ...
      printf(("radar_pi: %s Gain: %d auto %d"), m_name.c_str(), value, autoValue);
      r = TransmitCmd(cmd, sizeof(cmd));
      break;
    }
    ...
    case CT_SEA: {
      ...
      printf(("radar_pi: %s Sea: %d auto %d"), m_name.c_str(), value, autoValue);
      r = TransmitCmd(cmd, sizeof(cmd));
      break;
    }
    ...
    case CT_RAIN: {  // Rain Clutter - Manual. Range is 0x01 to 0x50
      ...
      printf(("radar_pi: %s Rain: %d"), m_name.c_str(), value);
      r = TransmitCmd(cmd, sizeof(cmd));
      break;
    }
    ...
    case CT_SIDE_LOBE_SUPPRESSION: {
      ...
      printf(("radar_pi: %s command Tx CT_SIDE_LOBE_SUPPRESSION: %d auto %d"), m_name.c_str(), value, autoValue);
      r = TransmitCmd(cmd, sizeof(cmd));
      break;
    }
    ...
    case CT_INTERFERENCE_REJECTION: {
      ...
      printf(("radar_pi: %s Rejection: %d"), m_name.c_str(), value);
      r = TransmitCmd(cmd, sizeof(cmd));
      break;
    }
    ...
    case CT_TARGET_EXPANSION: {
      ...
      printf(("radar_pi: %s Target expansion: %d"), m_name.c_str(), value);
      r = TransmitCmd(cmd, sizeof(cmd));
      break;
    }
    ...
    case CT_TARGET_BOOST: {
      ...
      printf(("radar_pi: %s Target boost: %d"), m_name.c_str(), value);
      r = TransmitCmd(cmd, sizeof(cmd));
      break;
    }
    ...
    case CT_LOCAL_INTERFERENCE_REJECTION: {
      ...
      printf(("radar_pi: %s local interference rejection %d"), m_name.c_str(), value);
      r = TransmitCmd(cmd, sizeof(cmd));
      break;
    }
    ...
    case CT_NOISE_REJECTION: {
      ...
      printf(("radar_pi: %s Noise rejection: %d"), m_name.c_str(), value);
      r = TransmitCmd(cmd, sizeof(cmd));
      break;
    }
    ...
    case CT_TARGET_SEPARATION: {
      ...
      printf(("radar_pi: %s Target separation: %d"), m_name.c_str(), value);
      r = TransmitCmd(cmd, sizeof(cmd));
      break;
    }
    ...
    case CT_DOPPLER: {
      ...
      printf(("radar_pi: %s Doppler state: %d"), m_name.c_str(), value);
      r = TransmitCmd(cmd, sizeof(cmd));
      break;
    }
    ...
    case CT_ANTENNA_HEIGHT: {
      ...
      printf(("radar_pi: %s Antenna height: %d"), m_name.c_str(), v);
      r = TransmitCmd(cmd, sizeof(cmd));
      break;
    }
  }

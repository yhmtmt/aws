    ...
    if (m_interface_count > 0) {
      ...
      for (addr = addr_list; addr; addr = addr->ifa_next) {
        if (VALID_IPV4_ADDRESS(addr)) {
          ...
          printf(("radar_pi: NavicoLocate scanning interface %s for radars"), m_interface_addr[i].FormatNetworkAddress());
          i++;
        }
      }
    }
...
void *NavicoLocate::Entry(void) {
  ...
  printf(("radar_pi: NavicoLocate thread starting"));
  ...
  while (!m_shutdown) {
    ...
    if (r < 0) {
      int err = errno;
      printf("radar_pi: NavicoLocate select %s"), strerror(err));
      UpdateEthernetCards();
      report_timeout = 0;
    }
    if (r > 0) {
      ...
      if (report_timeout >= PERIOD_UNTIL_CARD_REFRESH) {
        printf(("radar_pi: NavicoLocate t=%d max=%d"), report_timeout, PERIOD_UNTIL_CARD_REFRESH);
        UpdateEthernetCards();
        report_timeout = 0;
      } else {
        report_timeout++;
  ...
  printf(("radar_pi: NavicoLocate thread stopping"));
  m_is_shutdown = true;
  return 0;
}
...
void NavicoLocate::ProcessReport(const NetworkAddress &radar_address, const uint8_t *report, size_t len) {
  ...
  if (report[0] == 01 && report[1] == 0xB2) {  // Common Navico message from 4G++
    if (m_pi->m_settings.verbose >= 2) {
      logBinaryData("NavicoLocate received RadarReport_01B2"), report, len);
    }
    ...
    printf(("radar_pi: Located radar IP %s [%s]"), radar_ipA.FormatNetworkAddressPort(), infoA.to_string());
    ...
    printf("radar_pi: Located radar IP %s [%s]"), radar_ipB.FormatNetworkAddressPort(), infoB.to_string());

    m_pi->FoundNavicoRadarInfo(radar_ipB, infoB);

#define LOG_ADDR_N(n) \
    printf(("NavicoLocate %s addr %s = %s"), radar_address.FormatNetworkAddress(), #n, FormatPackedAddress(data->addr##n));
  ...
  logBinaryData(("NavicoLocate received unknown message"), report, len);
  }
}
...
void NavicoLocate::WakeRadar() {
  ...
  for (size_t i = 0; i < m_interface_count; i++) {
    ...
    if (sock != INVALID_SOCKET) {
      ...
      {
        printf(("Sent wake command to radar on %s"), m_interface_addr[i].FormatNetworkAddress());
      }
      else
      {
	printf(("Failed to send wake command to radars on %s"), m_interface_addr[i].FormatNetworkAddress());
      }
      closesocket(sock);
    }
  }

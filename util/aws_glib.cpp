		      ...
		      glm::vec4 & _clr, glm::vec4 & _bkgclr)
{
  ...
  float
    uend = (float)((double)spoke_len_max / (double)1024),
  ...
  */
  ...
  vertices[2].u = vertices[3].u = uend;  
  vertices[0].v = vertices[2].v = 0.0f;
  vertices[1].v = vertices[3].v = vend;
			      ...
			      const unsigned char * _line)
{
  ...
  memcpy((void*)texture_buffer.ptr<unsigned char>(_bearing),
	 _line, sizeof(unsigned char) * _len);
...
void c_gl_radar::update_done()
{
  glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, spoke_len_max, spokes, GL_RED,
...
float c_gl_text_obj::parse_float(ifstream & ifile, char * str_var,
				 int & len_str_var)
{
  len_str_var = 0;
  while (1){
...
c_gl_text_obj::c_gl_text_obj() : vao(0), vertices(NULL), texcoord(NULL),
				 indices(NULL), bupdated(false)
{
}
...
bool c_gl_line_obj::init(GLuint _modeloc, GLuint _posloc, GLuint _Mmvploc,
			 GLuint _clrloc, unsigned int _buffer_size)
{
...
bool c_gl_point_obj::init(GLuint _modeloc, GLuint _posloc,
			  GLuint _Mmvploc, GLuint _clrloc)
{
  modeloc = _modeloc;
  posloc = _posloc;
  Mmvploc = _Mmvploc;
